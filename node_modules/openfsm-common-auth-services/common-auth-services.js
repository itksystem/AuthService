...
const SQL = {
  USER: {    
    DISABLE_TWO_FACTOR : `UPDATE  pa_auth_service.two_factor_user_relations 
     SET blocked = now()
     WHERE user_id=$1
     RETURNING *
    `,
    
    DISABLE_PIN_CODE : `UPDATE  pa_auth_service.users
     SET pin_code = null
     WHERE id=$1
     RETURNING *
     `,

     SET_PIN_CODE : `UPDATE  pa_auth_service.users
     SET pin_code = $2
     WHERE id=$1
     RETURNING *
     `,

     REMOVE_PIN_CODE: `UPDATE  pa_auth_service.users
     SET pin_code = null
     WHERE id=$1
     RETURNING *
     `,

    SET_TWO_FACTOR : `INSERT INTO pa_auth_service.two_factor_user_relations 
    ...
    case 
      ...
      from pa_auth_service.two_factor_user_relations tfur 
        left join pa_auth_service.two_factors tf on (tfur.factor_id = tf.id and tf.id is not null )
      where user_id = $1 and tfur.blocked is null`,

    GET_SECURITY_FACTOR_KEY  : `select * from pa_auth_service.two_factor_user_relations where user_id = $1 and blocked is null`,

    GET_PIN_CODE_FACTOR :`select pin_code as "pinCode" from pa_auth_service.users where id = $1`,
    GET_TWO_FACTOR  : `select * from pa_auth_service.two_factor_user_relations where user_id = $1 and blocked is null`,
    GET_TWO_FACTOR_LIST  : `select id as "questionId", factor_text as "question" from pa_auth_service.two_factors`,
    ...
    TELEGRAM_CREATE: `INSERT INTO pa_auth_service.users ( password, verification_code) VALUES ($1, $2) RETURNING id;`,
    ...
    GET_ME : `SELECT     
    is_email_confirmed as "confirmed"
    FROM pa_auth_service.users 
    WHERE id = $1`,
    GET_EMAIL_CONFIRMED : `SELECT is_email_confirmed as confirmed  FROM pa_auth_service.users WHERE id = $1`,
    SET_EMAIL_CONFIRMED : `UPDATE pa_auth_service.users SET is_email_confirmed=NOW() WHERE id = $1`,
    SET_EMAIL_UNCONFIRMED : `UPDATE pa_auth_service.users SET is_email_confirmed=NULL WHERE id = $1`,
...
const MESSAGES = {
   RU : {
    ...
    EMAIL_CHANGED: 'Email был изменен. Необходимо его подтвердить.',
