...
exports.setTwoFactor = async (userId, factorId, factorText, factorKey) => {
  try {
    const result = await new Promise((resolve, reject) => {
      db.query(common.SQL.USER.SET_TWO_FACTOR, [userId, factorId, factorText, factorKey], (err, result) => {
        if (err) {
          logger.error('Ошибка при получении кода верификации:', err);
          return reject(err);
        }
