...
const {
  ...
  RABBITMQ_PAYMENT_ACCOUNT_CREATE_QUEUE, RABBITMQ_CHANGE_PIN_CODE_QUEUE, 
  RABBITMQ_MAIL_QUEUE, RABBITMQ_TWO_PA_CHANGE_STATUS_QUEUE  } = process.env;
...
// Установка пин-кода
exports.enablePinCode = async (userId, pinCodeHash) => {
  try {
    const result = await new Promise((resolve, reject) => {
      db.query(common.SQL.USER.SET_PIN_CODE, [userId, pinCodeHash], (err, result) => {
        if (err) {
          logger.error('setPinCode:', err);
          return reject(err);
        }
        resolve(result); 
      });
    ...
    logger.error('Ошибка в setPinCode:', error);
    throw error;
  }
};

// Удаление пин-кода removePinCode
exports.disablePinCode = async (userId) => {
  try {
    const result = await new Promise((resolve, reject) => {
      db.query(common.SQL.USER.REMOVE_PIN_CODE, [userId], (err, result) => {
        if (err) {
          logger.error('removePinCode:', err);
          return reject(err);
        }
        resolve(result); 
      });
    ...
    logger.error('Ошибка в removePinCode:', error);
    throw error;
  }
};
...
startConsumer(CHANGE_PIN_CODE_QUEUE, async (msg) => { 
  try {
    console.log('CHANGE_PIN_CODE_QUEUE = >',msg);  
    switch(msg.action){
      case 'PIN_CODE_DISABLE' : {
        await exports.disablePinCode(msg.user_id ?? null);
        break;
      }
      case 'PIN_CODE_ENABLE' : {
        const pinCodeHash = await bcrypt.hash(msg.pin_code.trim().toLowerCase(), 10);
        await exports.enablePinCode(msg.user_id, pinCodeHash);
        break;        
      }
    }   
