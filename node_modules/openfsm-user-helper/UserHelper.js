...
exports.telegramCreate = async (telegramId, hashedPassword) => {
  if (!telegramId || !hashedPassword) {
    return Promise.reject(new Error('telegramId и пароль обязательны.'));
  }
  try {
    const verificationCode = exports.verificationCode();
    const result = await new Promise((resolve, reject) => {
      db.query(common.SQL.USER.TELEGRAM_CREATE, [telegramId, hashedPassword, verificationCode], (err, result) => {
        if (err) {
          logger.error(err);
          return reject(err);
        }
  ...
  try {
    const result = await new Promise((resolve, reject) => {
      db.query(common.SQL.USER.GET_CONFIRMED, [userId], (err, result) => {
        ...
        resolve(result.rows[0]?.isConfirmed || false); // Предполагается, что поле isConfirmed
      });
    });
    return result;
  } catch (error) {
